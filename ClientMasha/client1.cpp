...
int main()
{
	...
	while (true)
	{
		...
		int bufferlength = userinput.size(); //Найти длину буфера
		Packet chatmessagepacket = P_ChatMessage; //Создать тип пакета: сообщение чата, отправляемое на сервер
		...
		send(Connection, userinput.c_str(), bufferlength, NULL); //Отправить буфер
		Sleep(10);
	}
	return 0;
}
